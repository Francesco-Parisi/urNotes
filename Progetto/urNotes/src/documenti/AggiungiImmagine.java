...
public class AggiungiImmagine extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String filenameImmagineDocumento = request.getParameter("filenameImmagineDocumento");
			...
			try {				
				...
				if(defaultImmagine == 1) { //Se deve essere primaria					
					...
					sql = "SELECT id_immagine "
						...
						+ "WHERE codice = "+codice+" AND is_default = 1 AND attivo = 1;";
					...
					if(result.wasNull()) {
						...
						if(rowCount == 0) {	
							...
							errore = "Esiste Gi&agrave; un'immagine primaria per questo documento.";
							risultato = 0;											
						}
					}
				}
				...
				if(continua == 1) {
					sql = "INSERT INTO documenti_immagini (codice, filename, is_default, attivo) VALUES (?, ?, ?, ?) ;";
					PreparedStatement  stmt = connDB.getConn().prepareStatement(sql);
					stmt.setInt(1, codice);
					stmt.setString(2, filenameImmagineDocumento);
