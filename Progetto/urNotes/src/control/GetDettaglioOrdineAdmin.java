				...
				try {
					...
					if(!result.wasNull()) {
						while(result.next()) {
							...
							sql = "SELECT email, codice_fiscale, nome, cognome "
								+ "FROM utenti "
								+ "WHERE id_utente = "+result.getInt("id_utente")+"; ";
							result1 = stmt1.executeQuery(sql);
	    					if(!result1.wasNull()) {
	    						while(result1.next()){
	    							cliente = result1.getString("email")+" - "+result1.getString("codice_fiscale")+" - "+result1.getString("nome")+" "+result1.getString("cognome");
	    						}
	    					}	
